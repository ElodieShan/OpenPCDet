...
from ...ops.mimic.hint_map import AttentionBlock
from .spconv_backbone import get_voxel_coords_inbox_dict
...
class SparseBasicBlock_SA(spconv.SparseModule):
    ...
    def __init__(
        ...
        self.remove_extra_feature = remove_extra_feature
    ...
    def forward(self, x):
        ...
        if self.sub_indices is not None:
            ...
            same_indices_high, same_indices_low, diff_indices = mimic_utils.get_same_indices(out.indices.long(), self.sub_indices.long(), return_diff_indices=True)

            if self.reset_extra_feature:
                out.features[diff_indices] = 0
                out.features[same_indices_high] = pooled_features[same_indices_low]
                out.features = self.bn2(out.features)
            elif self.remove_extra_feature:
                out.indices = out.indices[same_indices_high]
                out.features[same_indices_high] = pooled_features[same_indices_low]
                out.features = out.features[same_indices_high]
                out.features = self.bn2(out.features)
                identity.features = identity.features[same_indices_high]
            else:
...
class VoxelResBackBone8x_ATTEN(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.conv_out_atten = AttentionBlock(embed_dim=128, num_heads=4, max_seq_len=5*200*176)
        self.num_point_features = 128
    ...
    def forward(self, batch_dict):
        ...
        if 'sub_multi_scale_3d_features' in batch_dict:
            same_index = mimic_utils.get_same_indices(out.indices, batch_dict['encoded_spconv_tensor'].indices, return_same_indices_low=False)[0]
            sub_conv_out_features = self.conv_out_atten(out.features, out.indices, same_index, batch_size)
...
class VoxelBackBone8x_ATTEN(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
    ...
    def forward(self, batch_dict):
        ...
        if 'sub_branch' in batch_dict:
            same_index = mimic_utils.get_same_indices(out.indices, batch_dict['sub_branch']['encoded_spconv_tensor'].indices, return_same_indices_low=False)[0]
            # sub_out = out.clone()
            # sub_out.indices = sub_out.indices[same_index]
            # sub_out.features = sub_out.features[same_index]
            sub_out = spconv.SparseConvTensor(
                features=out.features[same_index].clone(),
                indices=out.indices[same_index],
        ...
        if 'sub_data' in batch_dict and batch_dict['sub_data']:
            out.indice_dict = None
            batch_dict.update({
                'encoded_spconv_tensor': out,
                'encoded_spconv_tensor_stride': 8
            })
...
class VoxelBackBone8x_ATTEN_v2(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.conv_out_atten = AttentionBlock(embed_dim=64, num_heads=4, max_seq_len=1*100*88, pos_shape=[1, 100, 88])

        # [200, 176, 2] -> [100, 88, 1]
        self.conv_downsample = block(128, 64, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv5', conv_type='spconv')

        # [100, 88, 1] -> [200, 176, 2]
        self.conv_upsample = block(64, 128, 3, norm_fn=norm_fn, indice_key='spconv5', conv_type='inverseconv')
    ...
    def forward(self, batch_dict):
        ...
        if 'sub_branch' in batch_dict:
            ...
            out_downsample = self.conv_downsample(out)
            sub_out = self.conv_downsample(sub_out)

            sub_out.features = self.conv_out_atten(out_downsample, sub_out, batch_size)
            # print("out_downsample:",out_downsample.features.shape)
            # print("out_downsample:",out_downsample.spatial_shape)
            # print("sub_out:",sub_out.features.shape)
            # print("sub_out:",sub_out.spatial_shape)

            sub_out_up = self.conv_upsample(sub_out)

            # print("sub_out upsample:",sub_out_up.features.shape)
            # print("sub_out upsample:",sub_out_up.spatial_shape)
            batch_dict['sub_branch'].update({
                'encoded_spconv_tensor': sub_out_up,
                'encoded_spconv_tensor_stride': 8,
            })
...
class VoxelBackBone8x_ATTEN_v3(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.conv_out_atten = AttentionBlock(embed_dim=64, num_heads=4, max_seq_len=2*200*176, pos_shape=[2, 200, 176])

        # [200, 176, 2]
        self.conv_channeldown = block(128, 64, 3, norm_fn=norm_fn, padding=1, indice_key='spconv_down2')

        # [200, 176, 2]
        self.conv_channelup = block(64, 128, 3, norm_fn=norm_fn, padding=1, indice_key='spconv_down2')
    ...
    def forward(self, batch_dict):
        ...
        if 'sub_branch' in batch_dict:
            ...
            out_downsample = self.conv_channeldown(out)
            sub_out = self.conv_channeldown(sub_out)

            sub_out.features = self.conv_out_atten(out_downsample, sub_out, batch_size)

            sub_out_up = self.conv_channelup(sub_out)
...
class SA_VoxelBackBone8x(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        SA_cfg = self.model_cfg.SA_LAYER
        reset_extra_feature = self.model_cfg.get('RESET_EXTRA_FEATURE', False)
        remove_extra_feature = self.model_cfg.get('REMOVE_EXTRA_FEATURE', False)
        self.sa_layer = SparseBasicBlock_SA(16, 16, norm_fn=norm_fn, indice_key='subm0',
                SA_cfg_src=SA_cfg, voxel_size=kwargs['voxel_size'], 
                point_cloud_range=kwargs['point_cloud_range'], 
                reset_extra_feature=reset_extra_feature, remove_extra_feature=remove_extra_feature)
    ...
    def forward(self, batch_dict):
        ...
        x = self.conv_input(input_sp_tensor)
        self.sa_layer.sub_indices = batch_dict['sub_voxel_coords'].int()

        x_input = self.sa_layer(x)
