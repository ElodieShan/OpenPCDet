...
class VoxelBackBone8x_ATTEN_v3(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.conv_out_atten = AttentionBlock(embed_dim=128, num_heads=4, max_seq_len=1*100*88, pos_shape=[1, 100, 88])

        # [200, 176, 2] -> [100, 88, 1]
        self.conv_downsample = block(128, 128, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv5', conv_type='spconv')

        # [100, 88, 1] -> [200, 176, 2]
        self.conv_upsample = block(128, 128, 3, norm_fn=norm_fn, indice_key='spconv5', conv_type='inverseconv')
...
#         # [200, 176, 2]
#         self.conv_channeldown = block(128, 64, 3, norm_fn=norm_fn, padding=1, indice_key='subm5')

#         # [200, 176, 2]
#         self.conv_channelup = block(64, 128, 3, norm_fn=norm_fn, padding=1, indice_key='subm6')
...
#         if 'sub_branch' in batch_dict:
#             same_index = mimic_utils.get_same_indices(out.indices, batch_dict['sub_branch']['encoded_spconv_tensor'].indices, return_same_indices_low=False)[0]
#             # same_index = mimic_utils.get_same_mask(out_downsample.indices, batch_dict['sub_branch']['encoded_spconv_tensor'].indices)

#             sub_out = spconv.SparseConvTensor(
#                 features=out.features[same_index].clone(),
#                 indices=out.indices[same_index].clone(),
#                 spatial_shape=out.spatial_shape,
#                 batch_size=batch_size
#             )
...
#             sub_out = self.conv_channelup(sub_out)
