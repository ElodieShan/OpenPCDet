...
from ...ops.mimic.hint_map import MappingBlock, AttentionBlock
from ...ops.pointnet2.pointnet2_stack.pointnet2_modules import StackMAXPOOLModuleMSG

class AnchorHeadTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, class_names, voxel_size, grid_size, point_cloud_range, predict_boxes_when_training, cls_score_thred=None):
        super().__init__()
            ...
            if feature_[:len('x_')] == 'x_' or feature_[-len('encoded_spconv_tensor'):] == 'encoded_spconv_tensor':
                ...
                if self.hint_gt_only:
                    ...
                    if self.random_select_bg:
                        ...
                        random.shuffle(rand_index_mask)
                        rand_index_mask = rand_index_mask if rand_index_mask.shape[0]<student_gt_coor_index.shape[0] else rand_index_mask[:student_gt_coor_index.shape[0]]
                        student_gt_coor_index = torch.cat((student_gt_coor_index,student_gt_diff_index[rand_index_mask]),0)
                     
                    if self.feature_sa_cfg is not None:
                        new_xyz = common_utils.get_voxel_centers(
                            student_feature_coor[student_gt_coor_index][:, 1:4],
                    ...
                    if self.seg_batch:
                        ...
                        for batch_id in range(batch_size):
                            ...
                            student_coor_index_batch = torch.cat((student_gt_coor_index_batch, student_gt_diff_index_batch),0)
                            teacher_coor_index_batch = torch.cat((teacher_gt_coor_index_batch, teacher_gt_diff_index_batch),0)
                            if self.feature_attention_cfg is not None:
                                feature_shape = teacher_feature.features[teacher_coor_index_batch].shape
                                # student_feature_shape = student_feature.features.shape
                                teacher_feature_attn = getattr(self, 'teacher_att_'+feature_)(teacher_feature.features[teacher_coor_index_batch].view(1,feature_shape[0],feature_shape[1]).permute(1,0,2), student_feature.features[student_coor_index_batch].view(1,feature_shape[0],feature_shape[1]).permute(1,0,2)).permute(1,0,2).view(feature_shape[0],feature_shape[1])
                                
                                hint_loss_src_batch += self.soft_hint_loss_func(student_feature.features[student_coor_index_batch], teacher_feature_attn).mean()
                            else:
                                hint_loss_src_batch += self.soft_hint_loss_func(student_feature.features[student_coor_index_batch], teacher_feature.features[teacher_coor_index_batch]).mean()
