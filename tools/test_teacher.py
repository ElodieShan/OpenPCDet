...
def eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False, save_iou=True, model_copy=None):
    ...
    eval_utils.eval_one_epoch(
        ...
        result_dir=eval_output_dir, save_to_file=args.save_to_file, save_iou=save_iou, use_sub_data=args.use_sub_data, model_copy=model_copy
    )
...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False, save_iou=True, model_copy=None):
    ...
    while True:
        ...
        tb_dict = eval_utils.eval_one_epoch(
            ...
            save_iou=save_iou, use_sub_data=args.use_sub_data, model_copy=model_copy
        ) # elodie
...
def main():
    ...
    if args.use_sub_data:
        test_set_sub = copy.deepcopy(test_set)
    ...
    with torch.no_grad():
        if args.eval_all:
            repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=dist_test, save_iou=args.save_iou, model_copy=model_sub) # elodie
        else:
            eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=dist_test, save_iou=args.save_iou, model_copy=model_sub) # elodie
