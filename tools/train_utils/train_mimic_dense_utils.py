...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        if model_teacher is not None: #elodie
            ...
            if use_sub_data:
                batch_dict_sub = {
                    'voxels': copy.deepcopy(batch_teacher['dense']['voxels']),
                    'voxel_coords': copy.deepcopy(batch_teacher['dense']['voxel_coords']),
                    'voxel_num_points': copy.deepcopy(batch_teacher['dense']['voxel_num_points']),
                    'batch_size': batch_teacher['batch_size'],
                    'gt_boxes': batch_teacher['gt_boxes'],
                    'sub_data':True,
                }
            ...
            batch_dict_sub = None
            if "dense" in batch: # dangerous
                if use_sub_data:
                    batch_dict_sub = {
                        'voxels': copy.deepcopy(batch['dense']['voxels']),
                        'voxel_coords': copy.deepcopy(batch['dense']['voxel_coords']),
                        'voxel_num_points': copy.deepcopy(batch['dense']['voxel_num_points']),
                    ...
                    if enable:
                        batch['points'] = batch['dense']['points_16lines']
                        batch['voxels'] = batch['dense']['voxels']
                        batch['voxel_coords'] = batch['dense']['voxel_coords']
                        batch['voxel_num_points'] = batch['dense']['voxel_num_points']
                        batch.pop('dense')  
                else:
                    batch.pop('dense')            
